...
const MAX_RETAINED_SLACK = 1024 * 1024
...
module.exports = exports = class WebSocket extends Duplex {
  ...
  _ondata(data) {
    ...
    while (
      ...
      this._buffer = this._buffered > 0 ? [retain(buffer.subarray(state.start))] : []
      this._frame = -1
    }
  }
...
exports.handshake = function handshake(req, cb) {
  ...
  req.on('upgrade', (res, socket, head) => {
    ...
    if (negotiated) return cb(negotiated)
...
// A subarray keeps the whole buffer it was taken from alive, so a tail left
// over from a large read is copied out rather than pinning everything that
// arrived alongside it.
function retain(buffer) {
  if (buffer.buffer.byteLength - buffer.byteLength <= MAX_RETAINED_SLACK) return buffer
