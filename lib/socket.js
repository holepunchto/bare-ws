...
module.exports = exports = class WebSocket extends Duplex {
  ...
  _onframe (frame) {
    if (frame.payload.length > 0 && !frame.mask === this._isServer) {
      this.destroy(this._isServer ? errors.EXPECTED_MASK() : errors.UNEXPECTED_MASK())

      return
    }

    if (frame.fin === false) {
      if (this._fragments.push(frame) === 1) return // First frame

      if (frame.opcode !== opcode.CONTINUATION) {
        this.destroy(errors.INVALID_OPCODE('Expected a continuation frame'))
      }
