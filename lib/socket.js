...
module.exports = exports = class WebSocket extends Duplex {
  constructor(url, opts = {}) {
    ...
    const {
      ...
      minBufferedChunks = MIN_BUFFERED_CHUNKS,
      minChunkAverage = MIN_CHUNK_AVERAGE,
    ...
    this._minBufferedChunks = minBufferedChunks
    this._minChunkAverage = minChunkAverage
  ...
  _ondata(data) {
    ...
    // The above bounds what is held at all; this bounds it against what the
    // peer has spent to make us hold it. Beyond a grace, each further chunk is
    // earned by `minChunkAverage` bytes, so trickling is refused early however
    // large the frame, while a frame arriving in large reads is never judged.
    if (
      this._minChunkAverage >= 0 &&
      (this._buffer.length - this._minBufferedChunks) * this._minChunkAverage > this._buffered
    ) {
      return this._fail(
        errors.CHUNKS_TOO_SMALL(
          `Chunks buffered into a frame must carry ${this._minChunkAverage} bytes on average`
        )
      )
    }
    ...
    while (
      ...
      try {
        this._onframe(Frame.decode(state, { maxPayload: this._remaining() }))
      } catch (err) {
  ...
  _onframe(frame) {
    ...
    if (frame.fin === false) {
      ...
      this._fragmented += frame.payload.length
