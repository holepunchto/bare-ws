...
// many of them a single frame is assembled from. Held no higher than what a
// frame of `MAX_PAYLOAD_LENGTH` needs when a peer's writes land one segment at
// a time, a retained chunk costing on the order of a kilobyte either way.
exports.MAX_BUFFERED_CHUNKS = 64 * 1024

// What a chunk costs to hold does not depend on what it carries, so a peer
// earns the parts it may keep us holding by the bytes it actually sends. This
// bounds that cost against the payload rather than against a count, which no
// ceiling low enough to matter can do without also refusing a large frame that
// arrives in large reads.
exports.MIN_CHUNK_AVERAGE = 256

// Chunks a peer is allowed before the average is held against it, so that a
// frame too small for what holding it costs to matter is never judged on one.
exports.MIN_BUFFERED_CHUNKS = 4096
