...
const { GUID, EOL, EOF, CLOSE_TIMEOUT } = require('./constants')
...
module.exports = exports = class WebSocketServer extends EventEmitter {
  constructor(opts = {}, onconnection) {
    ...
    this._socketOptions = { maxPayload, maxFragments, maxBufferedChunks, idleTimeout }
  ...
  close(cb) {
    ...
    for (const socket of this._handshakes) destroySoon(socket)
    ...
    for (const connection of this._connections) {
      ...
      const timer = setTimeout(() => connection.destroy(), CLOSE_TIMEOUT)
  ...
  _onupgrade(req, socket, head) {
    ...
    this._handshakes.add(socket)

    socket.on('close', () => this._handshakes.delete(socket))
