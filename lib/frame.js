...
module.exports = exports = class Frame {
  constructor(opcode, payload = EMPTY, opts = {}) {
    // A frame that carries nothing may be given its options in place of a
    // payload. Anything backed by bytes is a payload, whether or not it is a
    // `Buffer`, since mistaking one for options drops it and the options both.
    if (payload !== null && typeof payload === 'object' && !ArrayBuffer.isView(payload)) {
      opts = payload
      payload = EMPTY
    }
...
exports.decode = function decode(state, opts = {}) {
  ...
  if (op >= opcode.CLOSE) {
    ...
    // Refused on the header alone, before any of the payload is buffered. A
    // control frame is left out of this, being bounded by its own limit and no
    // part of the message a caller budgets for.
